...
;(function (window, document, $, undefined) {
    ...
    var defaults = {
        ...
        // If 'auto' - opacity will change if image and thumbnail have different aspect ratio
        opacity : 'auto',
        ...
        image : {
            ...
            // Requires predefined width and height
            preload : true,
        ...
        tpl	: '<div class="fancybox-container" role="dialog">' +
    ...
    var requestAFrame = (function() {
        ...
        var FancyBox = function( items, opts, index ) {
            var self = this;
        ...
        $.extend(FancyBox.prototype, {
            ...
            init : function( items ) {
                ...
                $('html').addClass('fancybox-noscroll').css('margin-right', measureScrollbar() );

                $container = $( self.opts.tpl )
                    .data( 'FancyBox', self )
                    .attr('id', 'fancybox-container-' + self.id)
                    .addClass( self.opts.mainClass )
            ...
            createGroup : function ( items ) {
                ...
                $.each(items, function( i, item ) {
                    ...
                    if ( $.isPlainObject( item ) ) {
                        ...
                        opts = 'options' in data ? data.options : opts;
                        ...
                        if ( $.type( self.opts.caption ) === 'function' ) {
                            ...
                            opts.caption = self.opts.caption.apply( item, [ self, obj ] );
            ...
            // Check if it is possible to pan image
            // ====================================
            ...
            // Check if image is scaled down and can be zoomed to actual size
            // ===============================================================
            ...
            // Scale image to actual size
            // ==========================

            goZoomIn : function( x, y ) {
                ...
                var canvasWidth  = parseInt( slide.$slide.width() );
                var canvasHeight = parseInt( slide.$slide.height() );
            ...
            getUpdatePos : function( slide ) {
                ...
                canvasWidth  = parseInt( slide.$slide.width() )  - ( margin[ 1 ] + margin[ 3 ] );
                canvasHeight = parseInt( slide.$slide.height() ) - ( margin[ 0 ] + margin[ 2 ] );
            ...
            update : function( immediately, andSlides, andContent ) {
                ...
                var leftValue = ( self.currPos * Math.floor( self.current.$slide.width() ) * -1  ) - ( self.current.pos * self.current.opts.space ) ;
            ...
            updateSlide : function( slide, andContent ) {
                ...
                $.fancybox.setTranslate( slide.$slide, { left : ( slide.pos * Math.floor( slide.$slide.width() )  ) + ( slide.pos * slide.opts.space) } );

                if ( andContent && $what ) {
                    to = self.getUpdatePos( slide );
            ...
            updateCursor : function( nextW, nextH ) {
                ...
                if ( nextW !== undefined && nextH !== undefined ) {
                    canZoomIn = nextW < 1 && nextH < 1;

                } else {
                    canZoomIn = self.canZoomIn();
                }
            ...
            loadSlide : function( slide ) {
                ...
                switch ( type ) {
                    ...
                    case 'swf':
                        ...
                        content = '<object id="fancyboxSwf" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="100%" height="100%"><param name="movie" value="' + slide.src + '"></param>';
                        embed   = '';

                        $.each( slide.swf || [], function( name, val ) {

                            content = content + '<param name="' + name + '" value="' + val + '"></param>';
                            embed   = embed + ' ' + name + '="' + val + '"';

                        });

                        content += '<embed src="' + slide.src + '" type="application/x-shockwave-flash" width="100%" height="100%"' + embed + '></embed></object>';

                        content = $('<div class="fancybox-content"></div>').append( content );

                        self.setContent( slide, content );
            ...
            setImage : function( slide ) {
                ...
                if ( !slide.opts.preload && slide.opts.width && slide.opts.height && ( slide.opts.thumb || slide.opts.$thumb ) ) {
            ...
            setContent : function ( slide, content ) {
                ...
                if ( slide.selector ) {
                    ...
                    content = $('<div>').html( content ).find( slide.selector );
                ...
                if ( slide.opts.smallBtn === true ) {
                    ...
                    slide.$content.remove( '.fancybox-close-small' ).prepend( '<button data-fancybox-close class="fancybox-close-small">×</button>' );
            ...
            cleanUp : function( e ) {
                ...
                if (instance) {
                    ...
                    $H.removeClass('fancybox-noscroll').css('margin-right', '');
            ...
            trigger : function( name ) {
                ...
                // Some events does not have current object
                if ( name === 'onInit' || name === 'onActivate' || name === 'afterClose' ) {
                    self.opts[ name ].apply( self, args );

                } else if ( current && $.isFunction( current.opts[ name ] ) ) {
                    current.opts[ name ].apply( current, args );
                }
        ...
        $.fancybox = {
            ...
            setTranslate : function( el, props ) {
                ...
                if ( props.left !== undefined || props.top !== undefined ) {
                    ...
                    if ( window.getComputedStyle( $( el ).get(0) ).getPropertyValue('transform') && !(document.documentMode && document.documentMode <= 9) ) {
        ...
        $.fn.fancybox = function (options) {
            ...
            this.off('click.fb-start touchstart.fb-start').on('click.fb-start touchstart.fb-start', {
                items   : this,
                options : options || {}
            }, _run);
	...
	var requestAFrame = (function() {
		...
		Guestures.prototype.ontouchstart = function(e) {
			...
			if (touches.length == 1) {
				...
				if (  ( this.currentWidth < this.canvasWidth + 1 && this.currentHeight < this.canvasHeight + 1 ) ) {
			...
			if ( touches.length == 1 ) {
				...
				if ( this.swiping ) {
					...
					if ( this.swiping !== 'x' && this.swiping !== 'y' && ( Math.abs(this.translateFromTranslatingX) > 2 || Math.abs(this.translateFromTranslatingY) > 2 ) ) {
						...
						if ( this.swiping === 'y' && this.instance.opts.touch.vertical === false ) {
							this.swiping = 'x';
						}
					}
				...
				if ( self.panning ) {
					...
					if ( Math.sqrt( this.translateFromTranslatingX * this.translateFromTranslatingX + this.translateFromTranslatingY * this.translateFromTranslatingY ) > 1 ) {
						...
						minTranslateX = Math.max(0, this.canvasWidth  / 2 - this.$image.width()  / 2 );
						minTranslateY = Math.max(0, this.canvasHeight / 2 - this.$image.height() / 2 );

						maxTranslateX = Math.min( this.canvasWidth  - this.$image.width(),  this.canvasWidth / 2  - this.$image.width()  / 2 );
						maxTranslateY = Math.min( this.canvasHeight - this.$image.height(), this.canvasHeight / 2 - this.$image.height() / 2 );
		...
		Guestures.prototype.ontouchend = function( e ) {
			...
			if ( swiping ) {
				...
				if ( swiping == 'y' && ( Math.abs( dY ) > 90 || (Math.abs( dY ) > 50 && dMs < 120) ) ) {
					...
					$.fancybox.animate( this.instance.$refs.slider, null, {
						top     : this.sliderPos.top + dY + ( speedY > 0 ? 1 : -1 ) * 100,
			...
			if ( this.newWidth / this.instance.current.width >  MAX_SCALE ) {
				...
				pos = this.instance.getUpdatePos( current );
			...
			$.fancybox.animate( this.$image, null, {
				top    : this.newOffsetY,
		...
		Guestures.prototype.ontap = function(e) {
			...
			if ( !$.fancybox.isTouch ) {
				...
				if ( self.instance.current.type == 'image' ) {
					...
					if ( self.instance.isZoomChanged() ) {

						self.instance.goZoomOut();


					} else if ( self.instance.canZoomIn() ) {

						self.instance.goZoomIn( x, y );
			...
			if (this.tapped) {
				...
				if (Math.abs(x - this.x) > 50) return this;
				if (Math.abs(y - this.y) > 50) return this;
		...
		$(document).on('onActivate.fb', function (e, instance) {
			...
			if ( instance.opts.touch && !instance.guestures_Enabled ) {
				instance.guestures_Enabled = new Guestures( instance );
			}

		});

		$(document).on('beforeClose.fb', function (e, instance) {
			if ( instance.guestures_Enabled ) {
				instance.guestures_Enabled.destroy();
			}
		});
...
;(function (document, $) {
	...
	$(document).on('onInit.fb', function(e, instance) {
		...
		if ( !!instance.opts.thumbs && !instance.fancyThumbs && instance.group.length > 1 && (
		    		( instance.group[0].type == 'image' || instance.group[0].opts.thumb ) &&
		    		( instance.group[1].type == 'image' || instance.group[1].opts.thumb )
			 	)
		   ) {
			instance.fancyThumbs = new FancyThumbs( instance );
		}

	});

	$(document).on('beforeMove.fb', function(e, instance, item) {
		var self = instance.fancyThumbs;
	...
	$(document).on('beforeClose.fb', function(e, instance) {
		...
		if ( instance.fancyThumbs && instance.fancyThumbs.isVisible && instance.opts.thumbs.hideOnClosing !== false ) {

			instance.fancyThumbs.close();

		}

		instance.fancyThumbs = null;
